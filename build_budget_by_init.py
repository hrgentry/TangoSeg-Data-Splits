...
# parents[2] is where this file sits in the study repository; at the top level of
# this archive no such ancestor exists and the import raised IndexError, so the
# "portable use" shown above could not run. Walk up to whichever ancestor holds
# benchmarks/ instead, and fall back to this file's own directory.
ROOT = next((p for p in Path(__file__).resolve().parents if (p / 'benchmarks').is_dir()),
            Path(__file__).resolve().parent)
BENCH = ROOT / 'benchmarks'
OUT = BENCH / 'budget_by_init.json'
# 均值口径与 Table 4 一致：取每个数据集里有 source-disjoint 划分的那个，所以
# Crack500 用母图互斥划分（crack500_mother），这样四集均值可以直接和表里的 Mean 列对上。
# 2026-09-14 订正：这里原先用发行划分，注释也写着"与 Table 4 一致"——那句在把表的
# Mean 列改成 parent-disjoint 之前是真的，之后就自相矛盾了。归档里的四集均值若与
# 研究仓不同口径，用本脚本重算出的增益就复现不了稿件报的数，而"能重算"正是本文件
# 存在的理由。发行划分那一格仍逐集报告（见 DATASETS），只是不进均值。
MEAN_DS = ['deepcrack', 'crack500_mother', 'camcrack789', 'crackmap']
DATASETS = MEAN_DS + ['crack500']
...
INIT = {
    ...
    'mambavision': 'None', 'scsegamba': 'None',
}
...
def caveats(n_pre, n_scr):
    """Cost-and-boundary notes, with every count derived from the two groups.

    The counts used to be literals ('Six models against nine', 'reuse 15
    models'). Adding a sixteenth model to the matrix left them at the old
    values while the numbers beside them moved, so they are derived now.
    """
    return [
        'One seed per cell: group differences include seed variability and are '
        'not matched contrasts. The repeated-run evidence is the '
        'three-architecture, three-seed budget comparison on DeepCrack.',
        'The groups are not randomly assigned and also differ in model family '
        'and scale. Counted GFLOPs include lower-bound entries. U-Net '
        'illustrates the pattern but cannot isolate initialization from '
        'capacity.',
        '%s models against %s: this describes the matrix, not a pretraining '
        ...
        % (_W[n_pre], _W[n_scr].lower()),
        'The %d cross-group comparisons reuse %d models; they are not %d '
        'independent runs or matched experimental pairs.'
        % (n_pre * n_scr, n_pre + n_scr, n_pre * n_scr),
    ]
...
def pair_wins(a, b):
    ...
    用配对数而不是 AUC 报数，是因为 "多少对里多少对" 读者可以自己核，而 0.96 这样的
    小数会让人以为精度比实际高。具体的组大小与配对数不写在这里 —— 它们随模型名单
    变，而这句只是在解释为什么选这种报数形式。2026-09-13 加第十六个模型后，原先
    写死的 "n=6/9"、"54 对里 52 对" 就全错了，而且随 v1.7.0 发了出去。
    """
...
def main():
    ...
    out = {
        ...
        'caveats': caveats(sum(1 for m in models if INIT[m] != 'None'),
                           sum(1 for m in models if INIT[m] == 'None')),
        'init_labels': INIT,
    ...
    best = max(move.values())
    # Report the tie rather than one silently chosen representative: with the
    # sixteenth model CarNet and U-Net both rise eight places, and a bare
    # 'largest_rise' would read as a unique maximum. Sorted by name so the
    # result does not depend on dict insertion order.
    tied = sorted(m for m in models if move[m] == best)
    top = tied[0]
    out['mean_rank_move'] = {
        'per_model': {m: {'rank_5': r5[m], 'rank_50': r50[m], 'move': move[m]}
                      for m in models},
        'largest_rise': {'model': top, 'move': best, 'tied_models': tied},
        'is_also_largest_absolute': abs(best) == max(abs(x) for x in move.values()),
    }
    ...
    pre_p = [pf[m]['params_M'] for m in models if INIT[m] != 'None']
    out['capacity_confound'] = {
        ...
        'pretrained_median_params_M': round(median(pre_p), 1),
        'scratch_median_params_M': round(
            median([pf[m]['params_M'] for m in models if INIT[m] == 'None']), 1),
        'unet_gflops': pf['unet']['gflops'],
        'unet_params_M': pf['unet']['params_M'],
        'unet_heavier_than_n_pretrained': sum(1 for x in pre_g if x < pf['unet']['gflops']),
        'unet_heavier_than_n_pretrained_by_gflops': sum(
            1 for x in pre_g if x < pf['unet']['gflops']),
        'unet_heavier_than_n_pretrained_by_params': sum(
            1 for x in pre_p if x < pf['unet']['params_M']),
        'n_pretrained': len(pre_g),
    }

    # 容量本身能不能分出同样的层? 按算力中位与按参数量中位各二分一次, 报同一套统计量。
    # 若容量分层与初始化分层同强, 就不能说"容量预测不了、初始化才预测得了"。
    def capacity_split(key):
        vals = {m: pf[m][key] for m in models}
        cut = median(list(vals.values()))
        light = [gm[m] for m in models if vals[m] < cut]
        heavy = [gm[m] for m in models if vals[m] >= cut]
        wins, total = pair_wins(light, heavy)
        return {'cut': round(cut, 2), 'n_light': len(light), 'n_heavy': len(heavy),
                'light_mean_gain': round(sum(light) / len(light), 4),
                'heavy_mean_gain': round(sum(heavy) / len(heavy), 4),
                'pairs_favouring_light': wins, 'pairs_total': total}

    out['capacity_stratification'] = {
        'by_gflops': capacity_split('gflops'),
        'by_params_M': capacity_split('params_M'),
        'comment': ('Median split of the %s configurations by each capacity proxy, '
                    % _W[len(models)].lower() +
                    'reporting the same gain statistics as the initialization split. '
                    'Gains are four-dataset means.'),
    }
